-- Fixed trigger that handles BOTH utilized_amount AND pending_amount
-- This prevents double-counting when expenses move from approved → paid
...
BEGIN
  ...
  IF target_category_id IS NOT NULL THEN
    ...
    SET 
      ...
      -- PENDING AMOUNT: Sum of all APPROVED expenses (waiting for payment)
      pending_amount = (
        SELECT COALESCE(SUM(total_amount), 0)
        FROM public.project_expenses
        WHERE budget_category_id = target_category_id
...
-- Recalculate all existing budget categories
DO $$
DECLARE
  cat_record RECORD;
BEGIN
  -- Update all budget categories to recalculate utilized_amount and pending_amount
  FOR cat_record IN 
...
-- Verification query: Check budget calculations
SELECT 

-- FIXED: Complete trigger that handles BOTH budget_categories AND project utilized_budget
-- Now ALWAYS updates project.utilized_budget for ANY paid expense

-- ============================================================================
-- PART 1: Update budget categories AND projects
-- ============================================================================
...
BEGIN
  ...
  -- CRITICAL FIX: ALWAYS update project utilized_budget (whether expense has category or not)
  IF target_project_id IS NOT NULL THEN
    
    UPDATE public.projects
    SET 
      -- UTILIZED BUDGET: Sum of ALL paid expenses (with or without budget_category_id)
...
BEGIN
  ...
  -- Step 2: Update all projects (SIMPLIFIED - just sum ALL paid expenses)
  FOR proj_record IN 
  ...
  LOOP
    ...
    SET 
      utilized_budget = COALESCE((
        -- ALL paid expenses for this project (regardless of budget_category_id)
        SELECT SUM(total_amount)
        FROM project_expenses
...
-- Check projects - NOW SHOWS ALL PAID EXPENSES
SELECT 
  p.name as project_name,
  p.total_budget,
  p.utilized_budget,
  (SELECT COUNT(*) FROM project_expenses WHERE project_id = p.id AND status = 'paid') as total_paid_count,
  (SELECT COALESCE(SUM(total_amount), 0) FROM project_expenses WHERE project_id = p.id AND status = 'paid') as actual_total_paid,
  (SELECT COUNT(*) FROM project_expenses WHERE project_id = p.id AND budget_category_id IS NULL AND status = 'paid') as without_category_paid_count,
  (SELECT COUNT(*) FROM project_expenses WHERE project_id = p.id AND budget_category_id IS NOT NULL AND status = 'paid') as with_category_paid_count
FROM projects p
ORDER BY p.created_at DESC
LIMIT 20;

-- Check if utilized_budget matches actual sum of paid expenses
SELECT 
  p.name as project_name,
  p.utilized_budget as stored_utilized,
  (SELECT COALESCE(SUM(total_amount), 0) FROM project_expenses WHERE project_id = p.id AND status = 'paid') as actual_paid_sum,
  p.utilized_budget - (SELECT COALESCE(SUM(total_amount), 0) FROM project_expenses WHERE project_id = p.id AND status = 'paid') as difference
FROM projects p
WHERE p.utilized_budget != (SELECT COALESCE(SUM(total_amount), 0) FROM project_expenses WHERE project_id = p.id AND status = 'paid')
